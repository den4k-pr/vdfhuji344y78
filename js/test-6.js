(function() {
  ...
  async function initVariant() {
    ...
    if (!variant) {
      console.log('%c No cookie variant, will fetch from server...', 'color:#0ff;font-weight:bold');
      try {
      ...
      // --- Встановлюємо cookie тільки якщо її не було ---
      setCookie(AB_COOKIE, variant, 365);
      console.log(`%c [ASSIGNED VARIANT] ${variant}`, 'color:#0f0;font-weight:bold');
    } else {
      console.log('%c Variant already in cookie, using cached value:', 'color:#0f0;font-weight:bold', variant);
    }
